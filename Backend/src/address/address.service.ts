...
export class AddressService {
    ...
    async newAddress(profileId, { city, state, country }: CreateAddressDTO): Promise<Address> {
        ...
        if (existingAddress.length > 0) {
            throw new HttpException('Address already existed', HttpStatus.BAD_REQUEST);
        }
        ...
        if (!newAddress) {
            throw new HttpException('Failed to add the new address', HttpStatus.BAD_REQUEST);
        }

        return newAddress;
    }

    async updateAddress(addressId: string, body: UpdateAddressDTO): Promise<{
