...
export class OffersService {
  ...
  async createOffer(profileId, shipmentId, { price }: CreateOfferDTO): Promise<{
    status: HttpStatus,
    message: string,
    newOffer: Offer
  }> {
    ...
    if (!newOffer) {
      throw new HttpException('Failed send your offer', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  ...
  async deleteOffer(profileId: string, offerId: string): Promise<{
    status: HttpStatus,
    message: string,
    deletedOffer: Offer
  }> {
    ...
    if (offer && offer.status !== OfferStatus.PENDING) {
      throw new HttpException(`Can\'t handle that request cuase offer it's ${offer.status}`, HttpStatus.BAD_REQUEST);
    }
